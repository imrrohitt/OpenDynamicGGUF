"""
Step 04 — Classify each tensor into a role + depth bucket.

Norm rules must run BEFORE attn_q/k/v patterns so names like
``attn_k_norm`` become ``norm``, not ``attn_k``.
"""
...
from .types import ClassificationResult, ClassifiedTensor

# Order matters: more specific / norm patterns first.
